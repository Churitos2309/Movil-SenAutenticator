...
import 'inicio/admin_menu.dart';
import 'Reportes/reportes_page.dart';
import 'Graficas/graficas_page.dart';
...
class _NavigationAdminState extends State<NavigationAdmin> {
  ...
  List<Widget> pages = [
    ...
    GraficasScreen(),   // Página de Gráficas
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: CurvedNavigationBar(
        ...
        buttonBackgroundColor: Colors.greenAccent,  // Fondo del ícono seleccionado
        backgroundColor: Colors.lightBlueAccent,  // Fondo del BottomNavigationBar
        animationCurve: Curves.easeInOut,  // Curva de animación
        animationDuration: const Duration(milliseconds: 600),  // Duración de la animación
        onTap: (index) {
          setState(() {
            _page = index;  // Actualiza el estado con la nueva página seleccionada
          });
          _pageController.jumpToPage(index);  // Navega a la página correspondiente
        },
      ),
    );
  }
